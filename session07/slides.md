...
- 会話の「所有者」をフロントからサーバに引き渡す回

---

## 今日のゴール

- 会話履歴を **SQLite に保存** して、リロード・再起動でも消えない状態を作る
- `messages` テーブル1つだけで実現する (まだ複数会話は無い)
- `init_db()` / `get_db_connection()` の使い方を身につける
- 次回 (第8回) の「複数会話」への足場を作る

---

## 今日の流れ

- 前半: なぜ永続化が必要か → スキーマ設計 → `init_db()` でテーブル作成
- 後半: 保存 / 読み込み / API へ流す処理を実装
...
- フロントが履歴を持つ世界から、**サーバが履歴を持つ** 世界へ移行した
- `messages` テーブル1つで十分実用になる
- `init_db()` / `get_db_connection()` のパターンは第8回でもそのまま使う
- フロントの責務が減って、UIに集中できるようになった
...
- `conversations` テーブルを追加
- `messages` に `conversation_id` 外部キーを生やす
- サイドバーから会話を切り替え・新規作成・削除
- 仕上げ + 発展テーマ (Tool use / RAG / エージェント) の概念紹介
- ついに **chat-app 完成**!
