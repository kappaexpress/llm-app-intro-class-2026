...
- ブラウザで動く **チャット画面** を作る
- AIが「**前の発言を覚えている**」状態を実現する
- ついでに **トークン** と **コンテキストウィンドウ** の感覚を掴む

---

## 今日の流れ

- 前半: チャットUIの設計 → HTML/CSS → fetch で1往復
- 後半: マルチターン会話 → 履歴をフロントで持つ → トークンの直感
- 演習: 1会話・メモリ保持(リロードで消える)版を完成させる
...
- チャットUI = メッセージリスト + 入力欄 + バブル
- LLM API は **ステートレス**: サーバは過去を覚えない
- 「覚えている」ように見せる = **毎回 messages 配列ごと送る**
- 会話が長くなる ≒ 送信トークンが増える
- `gpt-5.4-nano` のコンテキストウィンドウは **400K** と広いが無限ではない

---

## 次回予告: 第7回 履歴の永続化 (SQLite)

問題: **リロードで会話が消える**

解決策: サーバ側で **SQLite** に保存する

- `messages` テーブルを作って発言を保存
- 起動時に過去ログを読み込んで画面に復元
- フロントから「履歴を持って毎回送る」 のはやめる
  (サーバが履歴を取り出してOpenAIに送る形に変える)
